...
(define NAMES (list "Kaly" "Andrea" "Tai"))
#| Question 2
...
|#
;purpose: is to multiply all the list numbers together
;signature:  list-of-numbers -> number
(define (LIST-PRODUCT afk)
  (cond
    [(empty? afk) 1]
    [(cons? afk) (* (car afk)
                    (LIST-PRODUCT(cdr afk)))]))
...
|#
;purpose: To ge the words capatalized
;signature: list-of-strings -> string
(define (all-caps str)
  (cond
    [(empty? str) '()]
    [(cons? str) (cons(string-upcase (car str))
                                (all-caps (cdr str)))]))
...
|#
;purpose: gives you numbers if there is a number over 10
; signature: list-of-numbers -> boolean
(define (has-large-numbers? jks)
  (cond
    [(empty? jks) #f]
    [(cons? jks) (or (>(car jks)10)
                     (has-large-numbers?(cdr jks)))]))
...
|#
;Student is a (make-student string int list-of-number)
;A student represents a student in a school
;- name is what the student goes by
;- id students number
;- list-of-grades is students grades in their classes
(define-struct student (name id list-of-grades))
(define amanda-grades (list 78 76 80 97))
(define amanda (make-student "Amanda" 8900 amanda-grades))
(define tai-grades (list 100 75 87 89))
(define tai (make-student "Tai" 2345 tai-grades))
